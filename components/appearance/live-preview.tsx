...
export function LivePreview({ settings, links }: LivePreviewProps) {
  const activeTheme =
    themePresets.find((t) => t.id === settings.theme) ?? themePresets[0];
  const activeAccent =
    accentColors.find((a) => a.id === settings.accent_color)?.value ??
  ...
  return (
    <Card>
      <CardHeader className="border-b">
        ...
        <CardDescription>
          As alterações são atualizadas automaticamente.
        </CardDescription>
      </CardHeader>
      <CardContent className="pt-5">
        ...
        >
          <Avatar size="lg" className="size-16">
            ...
            />
            <AvatarFallback>
              {settings.display_name.slice(0, 2).toUpperCase()}
            </AvatarFallback>
          </Avatar>
            ...
            {visibleLinks.length === 0 ? (
              ...
              visibleLinks.map((link) => {
                ...
                return (
                  ...
                  >
                    {iconMeta && (
                      <iconMeta.Icon className="size-3.5 shrink-0" />
                    )}
          ...
          {settings.show_branding && (
            <span className="text-[10px] opacity-50">Made with Linkfee</span>
          )}
        </div>
      </CardContent>
    </Card>
