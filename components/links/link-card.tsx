...
export function LinkCard({ link, onToggle, onEdit, onDelete, onDuplicate }: LinkCardProps) {
  ...
  return (
    <Card
      ref={setNodeRef}
      style={style}
      className={cn('transition-opacity', !link.enabled && 'opacity-60', isDragging && 'z-10 opacity-80 shadow-lg')}
    >
      <div className="flex items-center gap-3 px-3">
        ...
        <div className="hidden items-center gap-0.5 md:flex">
          <Button variant="ghost" size="icon-sm" onClick={() => onEdit(link)} aria-label="Edit link">
            <Pencil className="size-4" />
          </Button>
          <Button variant="ghost" size="icon-sm" onClick={() => onDuplicate(link.id)} aria-label="Duplicate link">
